...
namespace Plutus.Xamarin
{
    ...
    public partial class BudgetsPage : ContentPage
    {
        ...
        public BudgetsPage(Services services)
        {
            _services = services;
            InitializeComponent();
        }
        ...
        private async void LoadAllBudgets()
        {
            ...
            {
                var stack = LoadBudget(budget);
                budgetStack.Children.Add(stack);
            }
        }

        private StackLayout LoadBudget(Budget budget)
        {

            var budgetSum = Convert.ToInt32(budget.Sum);
            var from = budget.From.ConvertToDate();
            var to = budget.To.ConvertToDate();
            var alreadySpent = 200; //is webapi gauti
            var leftToSpend = budgetSum - alreadySpent;
